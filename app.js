...
const translations = {
  en: {
    ...
    resource_search_placeholder: 'Search resources by title or description…',
    no_resources_match_search: 'No resources match your search.',
    ...
    staff_broadcast_preview_heading: 'Live preview',
    staff_broadcast_draft_banner_text: 'You have an unsaved draft from %s.',
    staff_broadcast_draft_restore_button: 'Restore draft',
    staff_broadcast_draft_discard_button: 'Discard',
    ...
    unfinished_registration_banner_title: 'Unfinished registration',
    ...
    group_by_table_toggle_label: 'Group by table',
  ...
  es: {
    ...
    resource_search_placeholder: 'Buscar recursos por título o descripción…',
    no_resources_match_search: 'Ningún recurso coincide con su búsqueda.',
    ...
    staff_broadcast_preview_heading: 'Vista previa en vivo',
    staff_broadcast_draft_banner_text: 'Tiene un borrador sin guardar de %s.',
    staff_broadcast_draft_restore_button: 'Restaurar borrador',
    staff_broadcast_draft_discard_button: 'Descartar',
    ...
    unfinished_registration_banner_title: 'Inscripción sin terminar',
    ...
    group_by_table_toggle_label: 'Agrupar por mesa',
...
app.get('/admin/catechists', requireAuth, requireRole('admin'), asyncHandler(async (req, res) => {
  ...
  // teaching children's classes and family faith leaders facilitating adult sessions
  // alike — not just the 'catechist' role literally.
  const catechists = await db.prepare(`
    SELECT id, full_name, email, phone, role FROM users
    WHERE role IN ('catechist', 'family_faith_leader') AND COALESCE(account_status, 'active') <> 'deleted'
    ORDER BY COALESCE(NULLIF(full_name, ''), email) ASC
  `).all();
...
app.post('/admin/catechists/:id/message', requireAuth, requireRole('admin'), asyncHandler(async (req, res) => {
  const catechist = await db.prepare(
    `SELECT id, full_name, email FROM users WHERE id = ? AND role IN ('catechist', 'family_faith_leader')`
  ).get(req.params.id);
